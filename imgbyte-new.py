...
def postVote(driver, setType, postid): #setType{ 1 for add downvote, 0 for remove} postid{ the image id of the post}
    session = requests.Session()
    selenium_cookies = driver.get_cookies()
    for cookie in selenium_cookies:
        session.cookies.set(cookie['name'], cookie['value'])
...
def commentVote(driver, setType, comID):
...
def ban_user(driver, user, stream, duration, banType):
    imgUrl(driver, f"bans?stream={stream}")
    #sets username
...
def comment_atComment(driver, post_id, com_id, text):
    text = json.dumps(text)
    imgUrl(driver, f"i/{post_id}?nerp=1741630780#com{com_id}")
    time.sleep(1.5)
...
def comment_atPost(driver, post_id, text):
    session = requests.Session()
    selenium_cookies = driver.get_cookies()
    for cookie in selenium_cookies:
        session.cookies.set(cookie['name'], cookie['value'])
...
def getBasicPosts(driver, stream, sort): #gets web elements of all posts in a page (hot/new)
    imgUrl(driver, f"m/{stream}?sort={sort}")
    time.sleep(2)
    posts = driver.find_elements(By.CLASS_NAME, "base-unit.clearfix")
    return posts

def getBasicComments(driver, post_id): #gets web elements of all comments on a page
...
def getCommentFlags(driver, stream=""):
    imgUrl(driver, f"comment-flags?stream={stream}")
    time.sleep(2)
...
def getApprovalQueue(driver, stream):
    imgUrl(driver, f"approval-queue?stream={stream}")
    time.sleep(2)
...
def create_post(driver, template, stream, title, nsfw, txt):
    imgUrl(driver, f"memegenerator/{template}")
...
driver = createWindow()
login(driver, 'ModBot', '64PSWDbit1!')
print(driver.token)
#print(del_comment(driver, 4890351, "other", "a", 60).text)
#print(get_unread_memechats(driver))
print(get_uid(driver, '32-bit'))
